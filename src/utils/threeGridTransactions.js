...
export function getAnalytics() {
  if (!transactionsData || transactionsData.length === 0) {
    return {
      ...
      suspiciousTransaction: { count: "Rs 0Cr", percentage: 0, extra: "Rs 0Cr" },
    };
  }

  // Find the earliest and latest months
  const timestamps = transactionsData.map((tx) => tx.timestamp);
  const firstYearMonth = timestamps.reduce((a, b) => (a < b ? a : b)).slice(0, 7);
  const currentYearMonth = timestamps.reduce((a, b) => (a > b ? a : b)).slice(0, 7);
  ...
  transactionsData.forEach((tx) => {
    ...
    if (!accountFirstTx.has(senderAccount) || timestamp < accountFirstTx.get(senderAccount)) {
      accountFirstTx.set(senderAccount, timestamp);
    }
    if (!accountFirstTx.has(receiverAccount) || timestamp < accountFirstTx.get(receiverAccount)) {
  ...
  accountFirstTx.forEach((timestamp) => {
    ...
    newAccountsPerMonth.set(yearMonth, (newAccountsPerMonth.get(yearMonth) || 0) + 1);
  });
  ...
  transactionsData.forEach((tx) => {
    ...
    totalTxAmountPerMonth.set(yearMonth, (totalTxAmountPerMonth.get(yearMonth) || 0) + amount);
    if (tx.label === "suspicious") { // Adjust this condition based on your data
      suspiciousTxAmountPerMonth.set(yearMonth, (suspiciousTxAmountPerMonth.get(yearMonth) || 0) + amount);
    }
  });
  ...
  const previousUsers = allYearMonths.length > 1 ? cumulativeUsersByMonth[allYearMonths[allYearMonths.length - 2]] : 0;
  const totalUsersPercentage = computePercentage(totalUsersCount, previousUsers);

  // Total Transaction
  const totalTxCount = formatCrores(cumulativeTotalTxByMonth[currentYearMonth]);
  const totalTxExtra = formatCrores(totalTxAmountPerMonth.get(currentYearMonth) || 0);
  const previousTotalTx = allYearMonths.length > 1 ? cumulativeTotalTxByMonth[allYearMonths[allYearMonths.length - 2]] : 0;
  const totalTxPercentage = computePercentage(cumulativeTotalTxByMonth[currentYearMonth], previousTotalTx);

  // Suspicious Transaction
  const suspiciousTxCount = formatCrores(cumulativeSuspiciousTxByMonth[currentYearMonth] || 0);
  const suspiciousTxExtra = formatCrores(suspiciousTxAmountPerMonth.get(currentYearMonth) || 0);
  const previousSuspiciousTx = allYearMonths.length > 1 ? cumulativeSuspiciousTxByMonth[allYearMonths[allYearMonths.length - 2]] : 0;
  const suspiciousTxPercentage = computePercentage(cumulativeSuspiciousTxByMonth[currentYearMonth] || 0, previousSuspiciousTx);
